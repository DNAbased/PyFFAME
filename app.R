...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      bsTooltip("db_genome", "Path to reference genome file in FASTA format."),
      ...
      bsTooltip("de_seq_1", "First added sequence, e.g. 5' of the feature."),
      textInput("de_seq_2", "Added sequence 2"),
      bsTooltip("de_seq_2", "Second added sequence, e.g. spacer between feature and barcode."),
      textInput("de_seq_3", "Added sequence 3"),
      bsTooltip("de_seq_3", "Third added sequence, e.g. 3' of the feature/barcode."),
      ...
      bsTooltip("enz_file_processed", "Path to processed enzyme file (columns: enzymes, site, expanded_sites). Expanded sites should be in a Python regex format, e.g. CTATCA[ACG]."),
      ...
      bsTooltip("enz_sites", "Expanded cut sites in a Python dictionary as Python regexes (e.g., ['GAATTC', 'CCTGCAGG'])."),
      numericInput("enz_cumul_cuts", "Total restriction sites expected",
                   value=2),
      bsTooltip("enz_cumul_cuts", "Total restriction sites expected in final feature, e.g. in the added sequences used for cloning."),
      numericInput("enz_cumul_cuts_bc", "Total restriction sites expected (in barcode plus the sequence before and after it)",
                   value=2),
      bsTooltip("enz_cumul_cuts_bc", "Total restriction sites expected in (barcode plus sequences right before and after it; used to screen the barcodes for excess restriction sites). Can be identical to total number of restriction sites expected."),
